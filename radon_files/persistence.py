...
def alpha_persistence(
    ...
    simplex_tree.persistence(homology_coeff_field=2, min_persistence=min_persistence)

    diagrams = []
    for dim in range(max_dim + 1):
        pairs = simplex_tree.persistence_intervals_in_dimension(dim)
        pairs = np.empty((0, 2)) if len(pairs) == 0 else np.sqrt(np.clip(np.asarray(pairs), 0, None))
        diagrams.append(pairs)
    return diagrams
...
def bootstrapped_persistence(
    ...
    all_diagrams = [[] for _ in range(max_dim + 1)]
    for _ in range(n_samples):
        idx = np.random.choice(len(points), size=min(subsample_size, len(points)), replace=False)
        sub = points[idx]
        dgms = alpha_persistence(sub, max_dim=max_dim) if use_alpha else witness_persistence(sub, max_dim=max_dim)
        for dim in range(max_dim + 1):
            all_diagrams[dim].append(dgms[dim])
    return [
        np.concatenate(all_diagrams[dim], axis=0) if all_diagrams[dim] else np.empty((0, 2))
        for dim in range(max_dim + 1)
    ]
def partitioned_persistence(
    points: np.ndarray | torch.Tensor,
    n_partitions: int = 4,
    max_dim: int = 1,
    use_alpha: bool = True,
    ...
    if isinstance(points, torch.Tensor):
        points = points.detach().cpu().numpy()
    labels = KMeans(n_clusters=n_partitions, n_init=3, random_state=0).fit_predict(points)
    all_diagrams = [[] for _ in range(max_dim + 1)]
    for k in range(n_partitions):
        chunk = points[labels == k]
        if len(chunk) < 10:
            continue
        dgms = alpha_persistence(chunk, max_dim=max_dim) if use_alpha else witness_persistence(chunk, max_dim=max_dim)
        for dim in range(max_dim + 1):
            all_diagrams[dim].append(dgms[dim])
    return [
        np.concatenate(all_diagrams[dim], axis=0) if all_diagrams[dim] else np.empty((0, 2))
