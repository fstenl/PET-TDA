...
def sample_events_2(image, proj, n_events):
    ...
    # Scale r to [0, cdf[-1]) using cdf[-1] rather than weights.sum() to avoid
    # floating-point discrepancies between cumsum and sum that could produce an
    # out-of-bounds index. dtype must match cdf for torch.searchsorted.
