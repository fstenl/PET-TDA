...
from scanner import get_mCT_scanner, sample_events_2
from gating import window_events, compute_window_pcfs, compute_distance_matrix
...
threshold = (D.max() + D.min()) / 2
cbar = fig.colorbar(im, ax=ax, label="L2 distance")
fig.tight_layout()
plt.show()
